...
// async getApplicationBy(appId: string): Promise<{ tasks: string[] }> {
//   const applicationData = await this.applicationModel
//     .findById(appId)
...
//   const tasks = Array.isArray(applicationData.) 
//     ? applicationData.tasks.map(taskId => taskId.toString()) 
//     : [];
...
async getActiveProjectsForEntrepreneur(entrepreneurId: string): Promise<any[]> {
  ...
  if (!projects.length) {
      throw new NotFoundException(`No projects found for entrepreneur with ID ${entrepreneurId}`);
  }
