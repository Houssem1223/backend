...
async countUsersByRole(roleName: string): Promise<number> {
  const role = await this.rolesService.getRoleByName(roleName); // Récupérer le rôle par nom
  if (!role) {
    throw new NotFoundException(`Role "${roleName}" not found`);
  }
  return this.userModel.countDocuments({ role: role._id }).exec(); // Compter les utilisateurs avec ce rôle
}
async getUserStatistics(): Promise<{ freelancersCount: number; entrepreneursCount: number }> {
  const freelancersCount = await this.countUsersByRole('Freelancer'); // Nom exact du rôle dans votre DB
  const entrepreneursCount = await this.countUsersByRole('Entrepreneur'); // Nom exact du rôle dans votre DB
  return { freelancersCount, entrepreneursCount };
}
