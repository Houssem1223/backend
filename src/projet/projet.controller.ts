...
export class ProjetController {
  ...
  @Post('save')
async saveProject(@Body() saveProjectDto: SaveProjectDto) {
  const { userId, projectId } = saveProjectDto;
  return this.projetService.saveProject(userId, projectId);
}
@Post('unsave')
async unsaveProject(@Body() saveProjectDto: SaveProjectDto) {
  const { userId, projectId } = saveProjectDto;
  return this.projetService.unsaveProject(userId, projectId);
}
@Get('saved/:userId')
async getSavedProjects(@Param('userId') userId: string) {
  return this.projetService.getSavedProjects(userId);
}
